...
The example featureset `input/featureset.MediaSearch_20210127.json` corresponds to the elasticsearch query generated by WikibaseMediaInfo as of Jan 27 2021
...
ATM the file assumes the ranklib file lives in `out/MediaSearch_20210127.tsv` - you can edit the code if you need to use a different one.
...
Running with no other options means the ranklib file gets shuffled randomly, then the first N rows are used for training the model, and the last 1000 rows are used for testing it.
